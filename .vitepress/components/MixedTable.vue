...
const mixedResults = computed<MixedResult[]>(() =>
	Object.values(groupBy(results[props.year], "CALL"))
...
const columns = [
	...
	columnHelper.accessor(
		(row) =>
			row.CW_QSO_COUNT_80m +
			row.PH_QSO_COUNT_80m +
			row.CW_QSO_COUNT_40m +
			row.PH_QSO_COUNT_80m,
		{
			header: "QSO count",
		},
	...
	columnHelper.accessor(
		(row) =>
			row.CW_MULT_80m + row.PH_MULT_80m + row.CW_MULT_40m + row.PH_MULT_40m,
		{
			header: "Multiplier",
		},
	),
...
<template>
	<Table
		...
		:columns="columns"
		:table-options="{
			initialState: { sorting: [{ id: 'TOTAL_SCORE', desc: true }] },
		}"
	/>
</template>
